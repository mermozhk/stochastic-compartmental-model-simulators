...
model <- function(edges,initial,parameters, rate_functions, final_time, plot=F){
  ...
  while(t<final_time && !identical(rates,rep(0,length(rates)))){
    ...
    next_event_time <- rexp(1,rate=sum(rates))
...
init$initial_value <- c(100000,5,0)
parameters <- c(.2,1/14)
...
Tf <- 200
y <- model(edges = edges,initial = init,parameters = parameters, rate_functions = c("infection_I","removal_I"),final_time = Tf,plot = F)
par(mfrow=c(2,2))
